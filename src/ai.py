...
class AI:
    ...
    def next_move(self, player):
        ...
        for move in self.get_possible_moves():
            ...
            score = self.minimax(position, depth, player_one)
    ...
    def minimax(self, position, depth, player_one): #TRUE if player 1, FAlSE if player 2
        ...
        for move in self.get_possible_moves():
            if self._board._check_valid_move(move) and self._board._check_four_connected(move, current_player):
                if current_player == 1:
                    return 1000 + depth
                else:
                    return -1000 - depth
        ...
        if player_one:
            value = -999999
            for move in self.get_possible_moves():
                position = self._board.make_move(move, current_player)
    ...
    def heuristic_evaluation(self, position):
        graph_current = self._create_graph_from_array(self._board._board, self._current_player)
        if self._current_player == 1:
            other = 2
        other = 1
        graph_other = self._create_graph_from_array(self._board._board, other)
